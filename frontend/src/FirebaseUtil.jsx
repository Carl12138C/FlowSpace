...
async function getter(uid,route){
    try{
        const response = await fetch(import.meta.env.VITE_SERVER + "/firebase/" + route+ "?" + new URLSearchParams({uid: uid}));
        if(response.ok){
...
async function setter(uid,route,data = '',isPost = true){
    try{
        ...
        const response = await fetch(import.meta.env.VITE_SERVER + "/firebase/" + route,
        {
            method: method,
            headers: {
